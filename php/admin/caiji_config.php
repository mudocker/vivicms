...
require_once("./caiji_config/del.php");
require_once("./caiji_config/yulan.php");
require_once("./caiji_config/savecollectid.php");
...
<?php if($temp){
	...
	<tr nowrap class="firstalt">
		...
		<td style="padding-left:20px"><a href="?ac=xiugai&id=<?php echo $vo['id'] ?>"><?php echo $vo['name'] ?></a></td>
		<td align="center"><?php echo $vo['collect_close']?'<a href="?ac=status&collectid=' . $vo['id'] . '&sid=0" title="点击开启"><font color="red">已关闭</font></a>':'<a href="?ac=status&collectid=' . $vo['id'] . '&sid=1" title="点击关闭"><font color="green">已开启</font></a>';
            ?></td>
		<td width="100" align="center"><a href="javascript:" onclick='alert("<?php echo!empty($vo['licence'])?str_replace(array("\r\n", "\r", "\n"), '\\n', $vo['licence']):'无';
            ?>");'>点我</a></td>
		<td width="100" align="center"><?php echo $vo['charset'] ?></td>
		<td width="150" align="center"><?php echo date('Y-m-d H:i:s', $vo['time']) ?></td>
		<td width="200" align="center"><a target="_blank" href="?ac=yulan&collectid=<?php echo $vo['id'] ?>">预览源代码</a>&nbsp;&nbsp;<a href="?ac=xiugai&id=<?php echo $vo['id'] ?>">修改</a>&nbsp;&nbsp;<a href="?ac=export&id=<?php echo $vo['id'] ?>">导出</a>&nbsp;&nbsp;<a href="?ac=import&id=<?php echo $vo['id'] ?>">导入</a>&nbsp;&nbsp;<a href="?ac=del&id=<?php echo $vo['id'] ?>" onClick="return confirm('确定删除?')">删除</a></td>
	</tr>
<?php }
    }else{
        ?>
...
##########';
    ...
    ?></textarea></td>
		...
		<tr nowrap class="firstalt">
			...
			<td>
				<label><input type="radio" id="replace_before_on" name="con[replace_before_on]" value="1" <?php if($caiji_config['replace_before_on'])echo " checked";
    ?> />开启</label>
				<label><input type="radio" id="replace_before_off" name="con[replace_before_on]" value="0" <?php if(!$caiji_config['replace_before_on'])echo " checked";
    ?> />关闭</label>
			</td>
		</tr>
    ...
    ?>>
			...
			<td><textarea name="con[replacerules_before]" style="height: 150px; width: 750px" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" ><?php echo _htmlspecialchars($caiji_config['replacerules_before']);
    ?></textarea></td>
		</tr>
...
.custom-header{
  ...
  color: #fff;
        ...
        foreach($arr as $k => $vo){
            ...
            ?>" <?php if(in_array($vo, $plusArr))echo " selected";
            ?>><?php echo $plusinfo['name'];
            ?></option>
			<?php }
    }
    ?>
			</select> <script type="text/javascript">$('.selectmultiple').multiSelect({ keepOrder: true,selectableHeader: "<div class='custom-header'>未使用的插件</div>",selectionHeader: "<div class='custom-header'>正在使用的插件</div>" });</script></td>
		</tr>
		</tbody>
    ...
    ?>" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" >&nbsp;<select onchange="$('#user_agent').val(this.value);">
    ...
    ?>" onFocus="this.style.borderColor='#00CC00'" onBlur="this.style.borderColor='#dcdcdc'" >&nbsp;<select name="con[ip_type]">
