        ...
        foreach($temp as $k => $vo){
            ...
            <tr nowrap class="firstalt">
                ...
                <td align="center"><?php if(@is_file(VV_DATA . '/plus/' . $vo['file'] . '/run.php')){
                        ?><a href="?ac=run&name=<?php echo $vo['file'] ?>">运行</a>&nbsp;&nbsp;&nbsp;&nbsp;<?php }
                    ?><?php if(@is_file(VV_DATA . '/plus/' . $vo['file'] . '/config.php')){
                        ?><a href="?ac=xiugai&name=<?php echo $vo['file'] ?>">配置</a><?php }else{
                        ?><font color="red">无配</font><?php }
                    ?>&nbsp;&nbsp;&nbsp;&nbsp;<a href="?ac=del&name=<?php echo $vo['file'] ?>" onClick="return confirm('确定删除?')">删除</a></td>
                <td>&nbsp;</td>
            </tr>
        <?php }
}else{
