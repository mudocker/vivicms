<?php
 require_once('data.php'); $v_config=require_once('../data/config.php'); require_once('checkAdmin.php'); $id=isset($_GET ['id'])?$_GET ['id']:''; set_time_limit(0); $ac=isset($_GET ['ac'])?$_GET ['ac']:''; if($ac==''){echo ADMIN_HEAD; ;echo '<body>
<div class="right">
		...
		<tr class="firstalt">
			<td colspan="2">
				<ul class="do_nav">
					...
					<a onClick="this.innerHTML=\'正在修正...\';" href="?ac=reset_domain" style="line-height:30px;padding-left:20px;color:red">修正域名绑定</a>&nbsp;
	...
	<tbody>
		...
		<form method="post" action="?ac=uptxt" enctype="multipart/form-data">
			...
			<tr nowrap class="firstalt webtype2">
				...
				<td><input name="indextitlefile" type="file">&nbsp;<font color="red">可选</font></td>
			</tr>
			<tr nowrap class="firstalt webtype2">
				<td width="260" class="son">∟ <b>网站名称库</b><br>
				<font color="#666666">网站名称txt库，每行一个网站名称</font></td>
				<td><input name="webnamefile" type="file">&nbsp;<font color="red">可选</font></td>
			</tr>
			<tr nowrap class="firstalt webtype2">
			...
			<tr nowrap class="firstalt">
				<td ><b>域名www重定向开关</b><br>
				<font color="#666666">开启后不带www的重定向到www</font></td>
				<td><select name="open301" >
					<option value="0" selected>关闭</option>
					<option value="1" >开启</option>
				</select></td>
			</tr>
			<tr nowrap class="firstalt">
				<td width="260"><b>是否特殊语种</b><br>
				<font color="red">如：日语、韩语、俄语等</font></td>
				<td><select name="no_convert" >
			...
			<tr nowrap class="firstalt">
				...
				<td><select name="domain_fan" >
					<option value="0"  selected>关闭</option>
					<option value="1" >开启</option>
				</select>&nbsp;&nbsp;<font color="red">将自动绑定泛域名*.xxx.com，注意泛域名要先解析好</font></td>
			</tr>
...
<table width="98%" cellspacing="1" cellpadding="4" border="0" class="tableoutline" id="config2" style="display:none">
	...
	<form method="post" action="?ac=set_gaoji"><input type="hidden" name="fanwei" value="" />
		<table width="98%" cellspacing="1" cellpadding="4" border="0" class="tableoutline">
			<tr nowrap  class="tb_head">
				<td colspan="2"><h2 style="color:#339900;font-weight:bold;">批量设置<font color="red">功能开关</font></h2></td>
			</tr>
			<tr nowrap class="firstalt">
				<td width="260"><b>选择功能</b><br>
				<font color="#666666">批量设置功能开关</font></td>
				<td><select name="key">
					<option value="301open">不带www的重定向到www</option>
					<option value="web_close">站点状态</option>
					<option value="rewrite">伪静态</option>
					<option value="replace">伪原创</option>
					<option value="hidejserror">屏蔽js错误</option>
					<option value="no_siteapp">禁止移动搜索转码</option>
					<option value="auto_get_search">自动替换搜索地址</option>
					<option value="no_convert">特殊语种</option>
				</select>&nbsp;&nbsp;<select name="value">
					<option value="1">开启/是</option>
					<option value="0" selected="">关闭/否</option>
				</select>  </td>
			</tr>
			<tr class="firstalt">
				<td width="260">&nbsp;</td>
	...
	<form method="post" action="?ac=set_theme"><input type="hidden" name="fanwei" value="" />
		<table width="98%" cellspacing="1" cellpadding="4" border="0" class="tableoutline">
			<tr nowrap  class="tb_head">
				<td colspan="2"><h2 style="color:#339900;font-weight:bold;">批量设置<font color="red">超级模板</font></h2></td>
			</tr>
			<tr nowrap class="firstalt">
...
';$themeArr=array(); $dir=VV_ROOT.'/template'; $themeArr=scandirs($dir); foreach($themeArr as $k=>$vo){$indexfile=$dir.'/'.$vo.'/index.html'; if($vo=='.' || $vo=='..' || !is_dir($dir.'/'.$vo) || !is_file($indexfile)){unset($themeArr[$k]); } } ;echo '			<tr nowrap class="firstalt">
				<td width="260"><b>选择模板风格</b><br>
				<font color="#666666">批量设置模板风格</font></td>
				<td><select name="theme_dir">
					<option value="">无</option>
				';foreach($themeArr as $k=>$vo){;echo '						<option value="';echo $vo;echo '" ';if ($caiji_config['theme_dir']==$vo) echo " selected";echo '>';echo $vo;echo '</option>
				';} ;echo '				</select>  </td>
			</tr>
			<tr class="firstalt">
				<td width="260">&nbsp;</td>
	...
	<form method="post" action="?ac=set_tj"><input type="hidden" name="fanwei" value="" />
		<table width="98%" cellspacing="1" cellpadding="4" border="0" class="tableoutline">
			<tr nowrap  class="tb_head">
				<td colspan="2"><h2 style="color:#339900;font-weight:bold;">批量设置<font color="red">统计代码</font></h2></td>
			</tr>
			<tr class="firstalt">
				<td width="260" valign="top"><b>统计代码</b><br>
				<font color="#666666">流量统计代码<br></font></td>
	...
	<form method="post" action="?ac=replace_fromtitle"><input type="hidden" name="fanwei" value="" />
		<table width="98%" cellspacing="1" cellpadding="4" border="0" class="tableoutline">
			<tr nowrap  class="tb_head">
				<td colspan="2"><h2 style="color:#339900;font-weight:bold;">批量替换<font color="red">目标站名称</font></h2></td>
			</tr>
			<tr nowrap class="firstalt">
	...
	<form method="post" action="?ac=replace_url"><input type="hidden" name="fanwei" value="" />
		<table width="98%" cellspacing="1" cellpadding="4" border="0" class="tableoutline">
			<tr nowrap  class="tb_head">
				<td colspan="2"><h2 style="color:#339900;font-weight:bold;">批量替换<font color="red">目标站地址</font></h2></td>
			</tr>
			<tr nowrap class="firstalt">
	...
	<form method="post" action="?ac=replace_kw"><input type="hidden" name="fanwei" value="" />
		<table width="98%" cellspacing="1" cellpadding="4" border="0" class="tableoutline">
			<tr nowrap  class="tb_head">
				<td colspan="2"><h2 style="color:#339900;font-weight:bold;">批量<font color="red">替换关键词</font></h2></td>
			</tr>
			<tr nowrap class="firstalt">
	...
	<form method="post" action="?ac=replace_domain"><input type="hidden" name="fanwei" value="" />
		<table width="98%" cellspacing="1" cellpadding="4" border="0" class="tableoutline">
			<tr nowrap  class="tb_head">
				<td colspan="2"><h2 style="color:#339900;font-weight:bold;">批量替换<font color="red">绑定的域名</font></h2></td>
			</tr>
			<tr nowrap class="firstalt">
...
';} if($ac=='delall'){@removedir(VV_DATA.'/config'); @mkdir(VV_DATA.'/config',0777,true); reset_domain(); ShowMsg("清除全部节点成功。",'?',1000); } if($ac=='replace_url'){extract($_POST); replace_cfg(array( 'from_url'=>array($oldurl,$newurl) ),$fanwei); ShowMsg("目标站地址批量替换成功~",'?',1000); } if($ac=='replace_zdy'){extract($_POST); replace_cfg(array( $key=>array($oldvalue,$newvalue) ),$fanwei); ShowMsg("自定义批量替换成功~",'?',1000); } if($ac=='replace_fromtitle'){extract($_POST); replace_cfg(array( 'from_title'=>array($oldtitle,$newtitle) ),$fanwei); ShowMsg("目标站名称批量替换成功~",'?',1000); } if($ac=='replace_kw'){extract($_POST); replace_cfg(array( 'web_name'=>array($oldkw,$newkw), 'web_seo_name'=>array($oldkw,$newkw), 'web_keywords'=>array($oldkw,$newkw), 'web_description'=>array($oldkw,$newkw), ),$fanwei); ShowMsg("关键词批量替换成功~",'?',1000); } if($ac=='replace_domain'){extract($_POST); replace_cfg(array( 'web_domains'=>array($olddomain,$newdomain) ),$fanwei); $domains_config=require_once(VV_DATA.'/domain_config.php'); if(isset($domains_config[$olddomain])){$domains_config[$newdomain]=$domains_config[$olddomain]; unset($domains_config[$olddomain]); arr2file(VV_DATA."/domain_config.php",$domains_config); } ShowMsg("域名批量替换成功~",'?',1000); } if($ac=='set_siftags'){extract($_POST); set_cfg('siftags',$siftags,$fanwei); ShowMsg("过滤批量替换成功~",'?',1000); } if($ac=='set_gaoji'){extract($_POST); set_cfg($key,$value,$fanwei); ShowMsg("批量设置高级功能成功~",'?',1000); } if($ac=='set_theme'){extract($_POST); set_cfg('theme_open',$theme_open,$fanwei); set_cfg('theme_dir',$theme_dir,$fanwei); ShowMsg("批量设置超级模板成功~",'?',1000); } if($ac=='set_zdy'){extract($_POST); set_cfg($key,$value,$fanwei); ShowMsg("自定义批量设置成功~",'?',1000); } if($ac=='set_tj'){extract($_POST); set_cfg('web_tongji',$tongji,$fanwei); ShowMsg("统计设置成功~",'?',1000); } if($ac=='reset_domain'){reset_domain(); ShowMsg("修正域名绑定成功~",'?',1000); } function set_cfg($key,$value,$fanwei=false){static $data; $filearr=glob(VV_DATA.'/config/*.php'); if($filearr){foreach($filearr as $k=>$vo){$fileid=intval(str_replace('.php','',basename($vo))); if(!isfanwei($fanwei,$fileid)){continue; } if(!isset($data[$vo])){$data[$vo]=require_once($vo); } $data[$vo][$key]=get_magic($value); arr2file($vo,$data[$vo]); } } } function replace_cfg($arg=array(),$fanwei=false){static $data; $filearr=glob(VV_DATA.'/config/*.php'); if($filearr){foreach($filearr as $k=>$vo){$fileid=intval(str_replace('.php','',basename($vo))); if(!isfanwei($fanwei,$fileid)){continue; } if(!isset($data[$vo])){$data[$vo]=require_once($vo); } $yes=false; foreach($arg as $kk=>$vv){if(stripos($data[$vo][$kk],$vv[0])!==false){$data[$vo][$kk]=str_ireplace($vv[0],$vv[1],$data[$vo][$kk]); $yes=true; } } if($yes){arr2file($vo,$data[$vo]); } } } } function isfanwei($fanwei, $fileid){if ($fanwei){if(stripos($fanwei,'-')>-1){list($min, $max)=explode('-', $fanwei); if ($min && $max){if ($min > $fileid || $fileid > $max){return false; } } }elseif($fanwei!=$fileid){return false; } } return true; } if($ac=='uptxt'){extract($_POST); $ymarr=uptxt('ymfile'); $urlarr=uptxt('urlfile'); if(empty($ymarr) || empty($urlarr)){ShowMsg("域名库、采集网站库不能为空~",'?',1000); } $tongji=get_magic($tongji); if($webtype==1){$kwarr=uptxt('kwfile'); if(empty($kwarr)){ShowMsg("关键词库不能为空~",'?',1000); } $webnamenum=max((int)$webnamenum,1); $kwnum=max((int)$kwnum,1); $desnum=max((int)$desnum,1); }else if($webtype==2){$indextitlearr=uptxt('indextitlefile'); $webnamearr=uptxt('webnamefile'); $keywordsarr=uptxt('keywordsfile'); $descriptionarr=uptxt('descriptionfile'); $kw_randarr=uptxt('kwfile_rand'); if(empty($indextitlearr) || empty($webnamearr)){} } $ymnum=max((int)$ymnum,1); $total_url=count($urlarr); $total_ym=count($ymarr); $ymdata=array(); foreach($urlarr as $k=>$vo){if(isset($ymarr[$k])){$ymdata[$ymarr[$k]]=$vo; unset($ymarr[$k]); } } if($total_url<$total_ym){if(($total_url*$ymnum)<$total_ym){$p=ceil($total_ym/($total_url*$ymnum)); }else{$p=ceil($total_ym/$total_url); } $temparr=$urlarr; for($i=0;$i<$p;$i++){$urlarr=array_merge($urlarr,$temparr); } shuffle($urlarr); foreach($ymarr as $k=>$vo){$ymdata[$vo]=$urlarr[$k]; } } $rule_cfg=require_once(VV_DATA.'/rule_cfg_tpl.php'); $domains_config=require_once(VV_DATA.'/domain_config.php'); $d_config=array(); $maxid=getconfig_maxid(); $success=0; $i=0; foreach($ymdata as $k=>$vo){$rootdom=$domains=$k; if($addwww==1){$domains='www.'.$domains.','.$domains; } if(isset($domains_config[$k]) || isset($domains_config['www.'.$k]) || isset($domains_config[$domains])){if($actype==1){continue; }else{$delid=''; if(isset($domains_config[$k])){$delid=$domains_config[$k]; unset($domains_config[$k]); } if(isset($domains_config['www.'.$k])){$delid=$domains_config['www.'.$k]; unset($domains_config['www.'.$k]); } if(isset($domains_config[$domains])){$delid=$domains_config[$domains]; unset($domains_config[$domains]); } @unlink(VV_DATA.'/config/'.$delid.'.php'); } } if($webtype==1){$web_seo_name=get_rand_arrstr($kwarr,$webnamenum); $keywords=$web_seo_name.','.get_rand_arrstr($kwarr,$kwnum-$webnamenum); $desription=$web_seo_name.','.get_rand_arrstr($kwarr,$desnum-$webnamenum); list($web_name,$tempaa)=explode(',',$web_seo_name); }else if($webtype==2){$web_seo_name=isset($indextitlearr[$i])?$indextitlearr[$i]:$indextitlearr[0]; $web_name=isset($webnamearr[$i])?$webnamearr[$i]:$webnamearr[0]; if($keywordsarr) $keywords=isset($keywordsarr[$i])?$keywordsarr[$i]:$keywordsarr[0]; if($descriptionarr) $desription=isset($descriptionarr[$i])?$descriptionarr[$i]:$descriptionarr[0]; } list($from_url,$from_title)=explode('|',$vo); if($from_url==''){continue; } if($kw_randarr){$kw_randarr=get_rand_arrstr($kw_randarr,11,''); for($n=1;$n<=10;$n++){$web_name=str_replace('{word'.$n.'}',$kw_randarr[$n],$web_name); $web_seo_name=str_replace('{word'.$n.'}',$kw_randarr[$n],$web_seo_name); $keywords=str_replace('{word'.$n.'}',$kw_randarr[$n],$keywords); $desription=str_replace('{word'.$n.'}',$kw_randarr[$n],$desription); } } if($domain_fan){$domains.=',*.'.$rootdom; } $setarr=array( 'web_name'=>$web_name, 'web_seo_name'=>$web_seo_name, 'web_keywords'=>$keywords, 'web_description'=>$desription, 'web_domains'=>strtolower($domains), 'from_url'=>$from_url, 'from_title'=>$from_title, 'web_404_url'=>'', 'web_tongji'=>get_magic($tongji), 'rewrite'=>$rewrite, 'web_close'=>$web_close, 'charset'=>'auto', 'time'=>time(), 'domain_fan'=>$domain_fan, 'domain_rules'=>'*', '301open'=>$open301, 'no_convert'=>$no_convert, ); $d_config[$domains]=$maxid; $success++; $ruledata=array_merge($rule_cfg,$setarr); $file=VV_DATA.'/config/'.$maxid.'.php'; arr2file($file,$ruledata); $maxid++; $i++; } $domains_config=@array_merge($domains_config,$d_config); arr2file(VV_DATA."/domain_config.php",$domains_config); ShowMsg("成功导入".$success."个域名~",'caiji_config.php',2000); } function get_rand_arrstr($arr,$num=1,$pf=','){if($num<1) return ''; shuffle($arr); $arr=array_slice($arr,0,$num); if($pf) return implode($pf,$arr); return $arr; } function getconfig_maxid(){$arr=@glob(VV_DATA.'/config/*.php'); if($arr){$arr=array_map('basename',$arr); $arr=array_map('intval',$arr); $id=max($arr)+1; }else{$id=1; } return $id; } function uptxt($name){if (!empty($_FILES[$name]['error'])){switch($_FILES[$name]['error']){case '1': $error='超过php.ini允许的大小'; break; case '2': $error='超过表单允许的大小'; break; case '3': $error='图片只有部分被上传'; break; case '4': $error='请选择图片'; break; case '6': $error='找不到临时目录'; break; case '7': $error='写文件到硬盘出错'; break; case '8': $error='File upload stopped by extension'; break; case '999': default: $error='未知错误'; } if($_FILES[$name]['error']!=4) ShowMsg($error,'-1',2000); } if(!empty($_FILES[$name]['name']) && !empty($_FILES[$name]['tmp_name']) && $_FILES[$name]['error']=='0' ){$file_name=$_FILES[$name]['name']; $tmp_name=$_FILES[$name]['tmp_name']; if(@is_uploaded_file($tmp_name) === false || !is_file($tmp_name)){ShowMsg($file_name."上传失败。",'-1',2000); } $arr=array(); if(is_file($tmp_name)){$arr=file($tmp_name); $arr=array_map('trim',$arr); } return $arr; } } ?>